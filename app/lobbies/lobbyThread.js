...
var statement = Sequences.getRandomSequence();
...
function sequenceGame(lobby){
    ...
    let leaderWordCount = 0;
    if(playerCount = 1){
        leaderWordCount = wordArray.length;
    }else{
        leaderWordCount = playerCount + (wordArray.length % playerCount);
    }
    ...
    PushNotifications.sendNotification(userTokens, "The game is on!", "Open the app to get your first challenge!", {Type: 'sequence game', id: game._id});
}
